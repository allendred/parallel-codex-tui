...
describe("package metadata", () => {
  ...
  it("documents public installation, requirements, and local data boundaries", async () => {
    ...
    expect(readme).toContain("v0.2.5` is available from [npm]");
    expect(readme).toContain("Main chat now extracts only the final Codex answer");
    ...
    expect(readme).toContain("1,281 tests across 127 files: 1,280 pass by default");
    ...
    expect(readme).toContain("package version `0.2.5` requires tag `v0.2.5`");
    expect(readme).toContain("Published tags such as `v0.2.4` are immutable");
  });
