...
describe("CLI help and version", () => {
  it("prints help and exits without starting the TUI", async () => {
    ...
    expect(stdout).toContain("--submit <request>");
    expect(stdout).toContain("--idempotency-key <key>");
    expect(stdout).toContain("--wait");
    expect(stdout).toContain("--cancel-run [id]");
    expect(stdout).toContain("--wait-run [id]");
    expect(stdout).toContain("--watch-run [id]");
    expect(stdout).toContain("--pause-feature <id>");
    expect(stdout).toContain("--cancel-feature <id>");
    expect(stdout).toContain("--resume-feature <id>");
    expect(stdout).toContain("--retry-task");
  ...
  it("prints the package version and exits without starting the TUI", async () => {
    ...
    expect(stderr).toBe("");
    expect(stdout.trim()).toBe("parallel-codex-tui 0.4.5");
  });
