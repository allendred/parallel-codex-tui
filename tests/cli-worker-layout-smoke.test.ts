...
describe("CLI worker layout smoke", () => {
  it("keeps the app header visible when worker logs fill a short terminal", async () => {
    ...
    try {
      ...
      expect(statusLine?.chunks.some((chunk) => chunk.style.backgroundColor === "ansi256(236)")).toBe(true);
      ...
      expect(chatSnapshot).toContain("> | Type a message · ^W logs · ^O attach");
  ...
  it("keeps worker controls on one line in a compact terminal", async () => {
    ...
    try {
      ...
      child.write("\x17");
      await waitForScreenText(() => screenWrites, screen, "logs · Pg · Tab · ^O · Esc");

      const snapshot = screen.snapshot();
      expect(snapshot).toContain("logs · Pg · Tab · ^O · Esc");
