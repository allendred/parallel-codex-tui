...
function lifecycleAgentSource(): string {
  return [
    ...
    "    fs.writeFileSync(path.join(dir, 'final-acceptance.json'), JSON.stringify({version:1,decision:'approved',summary:'game integration verified',acceptance:[{criterion_id:'A-001',status:'passed',evidence:'game.js is present in the final snapshot'}],changed_paths:['game.js']}));",
    "    console.log('final judge done');",
    "    return;",
    "  }",
    "  if (role === 'judge') {",
