...
function featureCancelAgentSource(): string {
  return [
    ...
    "    fs.writeFileSync(path.join(dir, 'final-acceptance.json'), JSON.stringify({version:1,decision:'approved',summary:'integration verified',acceptance:[{criterion_id:'A-001',status:'passed',evidence:'alpha and beta are integrated'}],changed_paths:['alpha.txt','beta.txt']}));",
