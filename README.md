...
- Each pending `commit_id` scopes deterministic replacement temp files to one live commit. Recovery completes an owned replacement only when its content and mode exactly match the integration snapshot and its target is still baseline, missing, or already integrated. A foreign or mismatched temp file blocks recovery and remains on disk for inspection.
