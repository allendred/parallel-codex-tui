...
- `--watch-run` follows durable events and incremental Worker logs in bounded 64 KiB chunks without claiming the TUI controller or acknowledging the result; unsafe log paths emit a structured warning instead of being read.
...
`--watch-run` first replays persisted Supervisor events and complete known Worker logs, then follows only newly appended output until the run settles. Worker logs are streamed in at most 64 KiB chunks without retaining their lifetime contents in watcher memory; a terminal run drains every already-persisted chunk before emitting its finish record. Reads are confined to the selected workspace's `.parallel-codex/sessions/` tree, including canonical-path checks against symlink escape. An invalid persisted log path produces one versioned warning record and its contents are not read. Text mode labels Router, status, Worker, log, and warning records; `--json` emits one compact, versioned JSON Lines record for each snapshot, event, Worker-output chunk, warning, and finish result. The finish record includes the terminal Main/Task summary or failure text. Unlike `--wait-run`, watch output intentionally contains Agent and project content, so review it before forwarding it to another system.
