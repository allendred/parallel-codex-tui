...
- Every complex run holds a task-owned `run-owner.json` lease, so another live TUI cannot concurrently retry or append a complex turn to the same task. Automated Worker processes run in owned process groups and persist `process.json` with their PID and OS process-start fingerprint while active; the role prompt is sent only after that ownership evidence is durable. If ownership recording fails, the process group is terminated before it receives the prompt and the Worker records `process-ownership-error`; normal exit removes a valid record.
